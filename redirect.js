document.addEventListener('DOMContentLoaded', function() {
    ...
    // Функция для установки правильного viewport
    ...
    document.body.addEventListener('click', function(e) {
        ...
        if (href.includes('minternational') || href.includes('api.whatsapp')) {
            ...
            if (isMobile()) {
                // Устанавливаем правильный viewport перед редиректом
                setMobileViewport();
                // Добавляем небольшую задержку для применения viewport
                setTimeout(() => {
                    window.location.href = whatsappUrl;
                }, 100);
            } else {
                // Для десктопов открываем в новом окне с заданными размерами
                const width = 500;
                const height = 600;
